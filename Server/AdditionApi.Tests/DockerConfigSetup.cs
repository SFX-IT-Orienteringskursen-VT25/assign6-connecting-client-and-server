...
public static class DockerConfigSetup
{
    private const string ContainerName = "sqlserver_integration_test";
    private const string ImageName = "mcr.microsoft.com/mssql/server";
    private const string ImageTag = "2022-latest";
    private const string Password = DbCredentials.Password;
    ...
    private static async Task WaitForSqlServerReadyAsync(TimeSpan timeout)
    {
        ...
        while (stopwatch.Elapsed < timeout)
        {
            try
            {
                await using var sqlConnection = new SqlConnection($"Server=localhost,1433;Database=master;User Id=sa;Password={Password};TrustServerCertificate=True;");
